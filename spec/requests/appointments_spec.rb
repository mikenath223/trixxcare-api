...
RSpec.describe "Appointments API", type: :request do
  ...
  describe "POST /appointments" do
    before { post "/api/appointments", params: { appointment: { user_id: user.id, doctor_id: doctor.id, date: Time.now } }, headers: auth_headers(user) }
    it "returns status code 200" do
      expect(response).to have_http_status(200)
    end
  ...
  describe "UPDATE /appointments/:id" do
    context "when the record exists" do
      before { patch "/api/appointments/#{id}", params: { appointment: { date: Time.now } }, headers: auth_headers(user) }
    ...
    context "when the record doesn't exist" do
      before { patch "/api/appointments/#{appointments.last.id + 1}", params: { appointment: { date: Time.now } }, headers: auth_headers(user) }
